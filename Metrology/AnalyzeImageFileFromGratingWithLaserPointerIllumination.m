...
% I=( A Jq (m2) / (  z))2
 ...
 for q = 1: N_orders                                            % loop through the orders (q) that are desired to represent the signal
    q_index( q )    =   q;                                        % need a index for referencing variables later
    I ( q, x_index )=   ( Amp /( lambda * z ) )^2 * besselj ( q - 1, ( m/2 ) )^2 * sinc( (( 2 * w  ) / ( lambda * z) ) * ( x - ( q - 1 ) * f_0 * lambda * z ) ).^2 * sinc( 2 * w * y / ( lambda * z ) )^2;  
  end
